...
def structural_change(mod1, mod2, k0, t_change, T_sim, npoints=300, figname=None):
    ...
    if t_change > 0:
        ...
        inv = optimize.minimize(
            fobj,
            x0=np.ones(t) * mod1.kss * mod2.delta,
            options={"disp": True},
            tol=1e-16,
        ).x
    ...
    for i in range(2):
        ...
        # Plot k0 locus
        ax[0, 1].plot(
            k_range, np.ones(npoints), linestyle="--", color=colors[i], label=labels[i]
        )
        # Plot q0 locus
        ax[0, 1].plot(
            k_range,
            [mods[i].lambda0locus(x) / mods[i].P for x in k_range],
            linestyle="--",
            color=colors[i],
        )
        # Plot steady state
...
sol = structural_change(
    mod1=Qmodel, mod2=Q_high_psi, k0=k0, t_change=t, T_sim=T, figname=figname
)
...
sol = structural_change(
    mod1=Q_high_tau, mod2=Qmodel, k0=k0, t_change=t, T_sim=T, figname=figname
)
# %% [markdown]
...
sol = structural_change(
    mod1=Qmodel, mod2=Q_high_itc, k0=k0, t_change=t, T_sim=T, figname=figname
)
# %% [markdown]
