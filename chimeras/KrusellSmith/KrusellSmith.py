...
KSAgentDictionary = {
    ...
    "MrkvArray": np.array(
        [[0.875, 0.125], [0.125, 0.875]]
    ),  # Transition probabilities for macroecon. [i,j] is probability of being in state j next
...
KSAgent.IncomeDstn[0] = [
    [
        ...
        np.array([ell_eg, ell_ug]),
    ],  # Agg state good
    [
        np.array([prb_eb, prb_ub]),
        np.array([p_ind, p_ind]),
        np.array([ell_eb, ell_ub]),
    ],  # Agg state bad
]
...
KSEconomyDictionary = {
    ...
    "MrkvArray": np.array([[0.875, 0.125], [0.125, 0.875]]),  # Transition probabilities
...
# %% {"code_folding": [], "jupyter": {"source_hidden": true}}
...
DiscFac_dstn = approxUniform(
    num_types, DiscFac_mean - DiscFac_spread, DiscFac_mean + DiscFac_spread
)[1]
...
[
    np.mean(KSEconomy_sim.aLvlNow[0]),
    np.mean(KSEconomy_sim.aLvlNow[1]),
    np.mean(KSEconomy_sim.aLvlNow[2]),
]
...
for i in range(len(MyTypes)):
    if i <= 2:
        plt.hist(
            np.log(KSEconomy_sim.aLvlNow[i]),
